...
void	Shader::Reload(bool deleteOld) {
	...
	// Clear uniform cache when reloading shader
	uniformCache.clear();
...
bool	Shader::LoadShaderFile(const string& filename, string &into)	{
	string fullPath = SHADERDIR + filename;
	ifstream	file(fullPath);
	string		textLine;

	cout << "Loading shader text from " << fullPath << "\n\n";

	if(!file.is_open()){
		cout << "ERROR ERROR ERROR ERROR: File does not exist!\n";
		cout << "  Attempted path: " << fullPath << "\n";
		return false;
	}
	int lineNum = 1; 
	while(!file.eof()){
...
GLint Shader::GetUniformLocation(const std::string& name) {
	...
	// Optional: warn if uniform not found (helpful for debugging)
	if (location == -1) {
		// Uncomment for debugging:
		// std::cerr << "Warning: Uniform '" << name << "' not found in shader program " << programID << std::endl;
	}
