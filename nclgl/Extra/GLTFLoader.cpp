...
bool GLTFLoader::Load(const std::string& filename, GLTFScene& intoScene) {
	...
	// Check file extension to determine if it's binary (.glb) or ASCII (.gltf)
	bool isBinary = (filename.length() >= 4 && filename.substr(filename.length() - 4) == ".glb");

	bool loadSuccess = false;
	if (isBinary) {
		loadSuccess = gltf.LoadBinaryFromFile(&model, &err, &warn, filename);
	} else {
		loadSuccess = gltf.LoadASCIIFromFile(&model, &err, &warn, filename);
	}
...
void GLTFLoader::LoadImages(tinygltf::Model& m, GLTFScene& scene, BaseState state, const std::string& rootFile) {
	...
	for (const auto& i : m.images) {
		...
		if (!i.image.empty()) {
			// Embedded texture (.glb): TinyGLTF already decoded image data
			tex = SharedTexture(OGLTexture::TextureFromData(
				(char*)i.image.data(), i.width, i.height, i.component).release());
		} else if (!i.uri.empty()) {
			// External file texture: load from disk
			std::filesystem::path imagePath;
			imagePath += std::filesystem::path(p.parent_path());
			imagePath.append(i.uri);
			tex = OGLTexture::TextureFromFile(imagePath.string());
		}
...
void GLTFLoader::LoadVertexData(tinygltf::Model& model, GLTFScene& scene, BaseState state ) {
	for (const auto& m : model.meshes) {
		...
		for (const auto& p : m.primitives) {
			...
			submesh.start = (int)firstIndex;
			submesh.count = (int)(vIndices.size() - firstIndex);
			submesh.base = (int)baseVertex;
		...
		SharedMesh sharedMesh = std::make_shared<OGLMesh>(*outMesh);

		scene.meshes.push_back(sharedMesh);
