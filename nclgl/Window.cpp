...
void	Window::ToggleFullscreen()	{
	...
	if(fullScreen) {
		...
		// Get monitor info for true fullscreen dimensions
		HMONITOR hMonitor = MonitorFromWindow(windowHandle, MONITOR_DEFAULTTONEAREST);
		MONITORINFO mi = { sizeof(mi) };
		GetMonitorInfo(hMonitor, &mi);

		// Store windowed position and size for restoration
		static WINDOWPLACEMENT g_wpPrev = { sizeof(g_wpPrev) };
		GetWindowPlacement(windowHandle, &g_wpPrev);
		...
		// Lock mouse and hide cursor for immersive experience
		LockMouseToWindow(true);
		ShowOSPointer(false);

		std::cout << "Switched to fullscreen mode ("
			<< (mi.rcMonitor.right - mi.rcMonitor.left) << "x"
			<< (mi.rcMonitor.bottom - mi.rcMonitor.top) << ")" << std::endl;
	}
	else {
		...
		SetWindowLong(windowHandle, GWL_STYLE,
			WS_OVERLAPPEDWINDOW | WS_POPUP | WS_VISIBLE | WS_SYSMENU | WS_MAXIMIZEBOX | WS_MINIMIZEBOX);
		...
		// Release mouse lock and show cursor
		LockMouseToWindow(false);
		ShowOSPointer(true);

		std::cout << "Switched to windowed mode ("
			<< (int)size.x << "x" << (int)size.y << ")" << std::endl;
	}
}